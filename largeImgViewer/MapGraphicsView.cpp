...
QSharedPointer<TileSource> MapGraphicsView::tileSource() const
{
    return _tileSource;
}

void MapGraphicsView::setTileSource(QSharedPointer<TileSource> tSource)
{
    _tileSource = tSource;
...
void MapGraphicsView::doTileLayout()
{
    ...
    const quint16 tileSize = _tileSource->getTileSize();
    const quint32 tilesPerRow = sqrt((long double)_tileSource->tilesNumOnZoomLevel(this->zoomLevel()));
...
void MapGraphicsView::resetQGSSceneSize()
{
    ...
    const quint64 dimension = sqrt((long double)_tileSource->tilesNumOnZoomLevel(this->zoomLevel())) * _tileSource->getTileSize();
